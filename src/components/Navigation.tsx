...
export default function Navigation({links}: Props) {
  ...
  return (
    <>
      <Box position={'fixed'} color={' white'} width={'100vw'} bg={useColorModeValue('gray.900', 'gray.100')} px={5}>
        <Flex h={16} alignItems={'center'} justifyContent={'space-between'}>
          ...
          <HStack spacing={8} alignItems={'center'}>
            <Box className='text-lg font-bold'><Link to="/">Developers Portfolio</Link></Box>
            <HStack as={'nav'} spacing={4} display={{ base: 'none', md: 'flex' }}>
			<LinksComponent links={links}/>
            </HStack>
          </HStack>
		  {user ? <Flex color={"black"} alignItems={'center'}>
            <Menu >
              <MenuButton
                as={Button}
                rounded={'full'}
